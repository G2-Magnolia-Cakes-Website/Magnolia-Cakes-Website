"""
This module was (shamelessly) copied directly from tinys3.auth

Changes from tinys3
-------------------
Parameters were added to AWS_QUERY_PARAMS to support multi-part uploads.
"""
...
class S3Auth(AuthBase):
    ...
    This code was copied from tinys3.auth.
    ...
    >>> from s3 import S3Auth
    >>> requests.put(
                url, 
                auth=S3Auth(access_key_id, secret_access_key),
                ...
                )
    """
    ...
    def __call__(self, r):
        ...
        Returns:    The request object, after we've updated some headers
        """

        # Generate the string to sign
        msg = self._string_to_sign(r)
        # Sign the string and add the authorization header
        r.headers['Authorization'] = "AWS %s:%s" % (
                self.access_key_id, 
                self._sign(msg))
    ...
    def _fix_content_length(self, request):
        ...
        request;  however, requests won't add this header, so we need to add 
        it ourselves.
    ...
    def _get_canonicalized_amz_headers(self, headers):
        ...
        Returns:    String with the canonicalized headers
    ...
    def _get_canonicalized_resource(self, request):
        ...
        Returns:        String the canoncicalized resource string.
        """
    ...
    def _string_to_sign(self, request):
        ...
        Returns:    String ready to be signed on
        """
        ...
        # If we have an 'x-amz-date' header, we'll try to use it instead of 
        # the date
