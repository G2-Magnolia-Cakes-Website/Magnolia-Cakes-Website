...
class Flavor(models.Model):
    ...
    def __str__(self):
        return f"{self.product_item.name} - {self.name}"
    ...
    def save(self, *args, **kwargs):
        if self.product_item:
            ...
            if self.product_type == ProductType.CUPCAKE:
                ...
                # Flavor is not for cupcake products, no Stripe logic here
                super(Flavor, self).save(*args, **kwargs)
        else:
            # Flavor is not linked to a product
            super(Flavor, self).save(*args, **kwargs)
    ...
    def delete(self, *args, **kwargs):
        if self.product_type == ProductType.CUPCAKE:
            ...
            try:
                stripe.api_key = settings.STRIPE_SECRET_KEY
                if self.product_item and self.product_id:
                    stripe.Product.modify(self.product_item.product_id, active=False)
